...
def make_targets_gene2idx(hgnc_data):
    ...
    alias_symbols["HLA-DR"] = ["HLA-DRA", "HLA-DRB1", "HLA-DRB2", "HLA-DRB3", "	HLA-DRB4", "HLA-DRB5", "HLA-DRB6", "HLA-DRB7", "HLA-DRB8", "HLA-DRB9"]
    ...
    for i, group_id in enumerate(unique_group_ids):
        ...
        for input_index in range(inputs_values.shape[1]):
            if (input_index % 1000) == 0:
                print(f"{i} {input_index} / {inputs_values.shape[1]} ({input_index/(inputs_values.shape[1]): .4f}) completed. elapsed time: {time.time() - start_time}")
            inputs_gene_name = inputs_gene_names[input_index].upper()
