...
class PrePostProcessing(object):
    ...
    def _preprocess(self, inputs_values, targets_values, metadata, test_inputs_values, test_metadata, fitting):
        ...
        if self.params["task_type"] == "multi":
            ...
            if fitting:
                inputs_targets_pair = np.load(os.path.join(self.params["data_dir"], "cite_inputs_targets_pair3g.npz"), allow_pickle=True)["mask"]
            ...
            if fitting:
                ...
                self.preprocesses["inputs_medians"].shape[0] == transformed_inputs_values.shape[1]
        ...
        if inputs_decomposer_class is not None:
            ...
            if self.params["task_type"] == "multi":
                binary_transformed_inputs_values = self.preprocesses["binary_inputs_decomposer"].transform(binary_transformed_inputs_values)
    ...
    def fit_preprocess(self, inputs_values, targets_values, metadata, test_inputs_values, test_metadata):
        self._preprocess(inputs_values=inputs_values, targets_values=targets_values, metadata=metadata, test_inputs_values=test_inputs_values, test_metadata=test_metadata, fitting=True)
        self.is_fitting = True
        return self

    def preprocess(self, inputs_values, targets_values, metadata):
        return self._preprocess(inputs_values=inputs_values, targets_values=targets_values, metadata=metadata, test_inputs_values=None, test_metadata=None, fitting=False)
